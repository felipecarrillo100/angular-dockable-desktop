...
([chapter 1](docs/manual/01-getting-started.md)).
...
| [`docs/PARITY.md`](docs/PARITY.md) | The API map to vdd, the test map, and every deliberate divergence (D1–D16, N1–N17) |
| [`docs/PROGRESS.md`](docs/PROGRESS.md) and [`docs/evidence/`](docs/evidence/) | One row and one written record per milestone gate |
| [`CHANGELOG.md`](CHANGELOG.md) | What changed |

## Repository layout

```
projects/angular-dockable-desktop/   the library (src/lib) and its Vitest suites (test/)
projects/demo/                       the full demo: 16 panel kinds, Monaco, Leaflet, markdown, 6 locales
projects/playground/                 the minimal harness the browser gates drive
docs/                                manual, ADRs, parity, plan, progress and evidence
...
npm test                  # the unit suite (Vitest, via the Angular CLI)
npm run lint              # angular-eslint
npm run build:lib         # the APF package, .d.ts and styles.css, into dist/

npm run demo              # build the library and serve the demo
npm run playground        # build the library and serve the harness

npm run gate -- M15       # the full release gate
npm run gate:selftest     # prove every gate rule can fail
npm run gate:sweep        # stub every module in turn; each must turn the suite red
```

`npm run gate -- M<n>` runs the standing gate — types, lint, the whole suite, the library build,
test counts, the CSS-prefix, API-surface and docs/API checks, the playground, demo and zone.js
builds — then the milestone's own rules and its real-Chrome browser gate, under zoneless and
zone.js. Gates are never edited to make a run pass.

## Continuous integration

Two workflows, written and ready, on Node 24:

- [`.github/workflows/gate.yml`](.github/workflows/gate.yml) runs `npm run gate -- M15` on every
  push and pull request, browser gates included (the runner image ships Chrome), and keeps the
  gate's evidence as an artifact.
- [`.github/workflows/pages.yml`](.github/workflows/pages.yml) builds the demo and publishes it to
